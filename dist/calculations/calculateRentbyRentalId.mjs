...
const calculateTotalRentalCharges1FromRentalId = async (rentalId) => {
    let totalCharge = 0;
...
const calculateTotalRentalCharges2FromRentalId = async (rentalId) => {
    let totalCharge = 0;
    const bookRented = await BooksRented.findById(rentalId);
    if (bookRented?.isActive) {
...
// Update the calculateTotalRentalCharges function
const calculateTotalRentalCharges3FromRentalId = async (rentalId) => {
    let totalCharge = 0;
    const bookRented = await BooksRented.findById(rentalId);
    if (bookRented?.isActive) {
        const bookdata = await Books.findById(bookRented.bookId);
